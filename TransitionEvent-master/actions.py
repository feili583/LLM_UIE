...
class Actions(object):
    ...
    left_relate = 'LEFT-RELATE'
    right_relate = 'RIGHT-RELATE'
    ...
    @staticmethod
    def make_oracle(tokens, triggers, ents, args, relations, with_copy_shift=True):
        '''
        ...
        rela_list :  [[entity1_start, entity1_end, entity2_start, entity2_end, relation_type]]  e.g. [[21, 21, 27, 28, 'type'],...]
        '''
        ...
        rela_dic = {(rel[0], rel[1], rel[2], rel[3]):rel for rel in relations}
            ...
            for j in range(tri_i - 1, -1, -1):
                ...
                if j in ent_dic:
                    ...
                    if key in arg_dic:
                        ...
                        cur_actions.append(Actions.left_pass + '-' + role_type)
        ...
        for ent_i in ent_dic:
            ...
            if with_copy_shift and ent_i in trigger_dic:
                if (ent_i, ent_i) in arg_dic:
                    ...
                    cur_actions.append(Actions.right_pass + '-' + role_type)
                    #cur_actions.append(Actions.event_gen + role_type)
                else:
                    cur_actions.append(Actions.no_pass)
            ...
            for j in range(ent_i - 1, -1, -1):
                ...
                if j in ent_dic:
                    start1, end1, ent_type1 = ent_dic[ent_i]
                    start2, end2, ent_type2 = ent_dic[j]
                    key1 = (start1, end1, start2, end2)
                    key2 = (start2, end2, start1, end1)
                    if key1 in rela_dic:
                        _, _, _, _, rela_type = rela_dic[key1]
                        cur_actions.append(Actions.left_relate + '-' + rela_type)
                    elif key2 in rela_dic:
                        _, _, _, _, rela_type = rela_dic[key2]
                        cur_actions.append(Actions.right_relate + '-' + rela_type)
                    else:
                        cur_actions.append(Actions.no_pass)
